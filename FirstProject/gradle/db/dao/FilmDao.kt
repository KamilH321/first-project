...
interface FilmDao {
    ...
    @Query("SELECT id, title, description, releaseYear, genre, country, userId FROM films WHERE id = :filmId")
    fun getFilmDataById(filmId: Int): FilmEntity?

    @Query("SELECT id, title, description, releaseYear, genre, country, userId FROM films WHERE userId = :userId")
